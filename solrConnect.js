...
solrConnectPromise.prototype = {
    ...
    getFieldList:function (){
        ...
        objRes = this.postRequest(connectionParams, '').then(function(objResponse){
            return new Promise(function(resolve,reject){
                this.callBack(objResponse, 'fieldList').then(function(objReturn){
    ...
    getLeader:function(){
        ...
        objRes = this.postRequest(connectionParams, '').then(function(objResponse){
            return new Promise(function(resolve,reject){
                this.callBack(objResponse, 'leader').then(function(objReturn){
                    if(objReturn.responseHeader.status == 0){
                        resolve(objReturn);
                    } else {
                        reject(objReturn)
        ...
        return new Promise(function(resolve, reject){
            ...
            if (objResponse.responseHeader.status == 400)
            {
                ...
                switch (parseResponse) {
                    ...
                    case 'data_only':
                        objReturn = objResponse.response;
                    break;
                    ...
                    case 'facets_only':
                        objReturn = objResponse.facet_counts.facet_fields;
                    break;

                    case 'data_facets':
                        objReturn = {data:{},facet_counts:{}};
                        objReturn.data = objResponse.response;
                        objReturn.facet_counts = objResponse.facet_counts.facet_fields;
                    break;

                    case 'record_count':
                        objReturn = objResponse.response.numFound;
                    break;
